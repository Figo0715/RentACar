...
namespace Business.Concrete
{
    public class RentalManager : IRentalService
    {
        ...
        {
            var rulesResult = BussinessRules.Run(CheckIfCarAvailableNow(carId));
            if (rulesResult != null)
            {
        ...
        public IResult Add(Rental rental)
        {
            ...
            {
                rulesResult = BussinessRules.Run(CheckIfCarAvailableBetweenSelectedDatesForReservations(rental.CarId, rental.RentDate, rental.ReturnDate),
            ...
            {
                rulesResult = BussinessRules.Run(CheckIfCarAvailableNow(rental.CarId),
                    DeliveryStatusShieldForAddOperation(rental));
            }
            if (rulesResult != null)
            {
        ...
        public IResult Update(Rental rental)
        {
            ...
            {
                rulesResult = BussinessRules.Run(CheckIfCarAvailableBetweenSelectedDatesForCurrentRentals(rental.CarId, rental.RentDate, rental.ReturnDate),
       ...
       // [TransactionScopeAspect]
        public IDataResult<int> Rent(RentPaymentRequestModel rentPaymentRequest)
        {
            ...
            if (creditCardResult.Success)
            {
                ...
                foreach (var rental in rentPaymentRequest.Rentals)
                {
                    ...
                    if (rental.RentDate > DateTime.Now) //Reservation
                    {
                        rulesResult = BussinessRules.Run(CheckIfCarAvailableBetweenSelectedDatesForReservations(rental.CarId, rental.RentDate, rental.ReturnDate), CheckIfReturnDateGreaterThanRentDate(rental.RentDate, rental.ReturnDate), CheckIfCustomerIdsAreEqual(rentPaymentRequest.CustomerId, rental.CustomerId));
                    ...
                    {
                        rulesResult = BussinessRules.Run(CheckIfCarAvailableNow(rental.CarId), CheckIfReturnDateGreaterThanRentDate(rental.RentDate, rental.ReturnDate), CheckIfCustomerIdsAreEqual(rentPaymentRequest.CustomerId, rental.CustomerId));
                    }
        ...
        {
            var rulesResult = BussinessRules.Run(CheckIfRentalIdExist(rentalId));
            if (rulesResult != null)
            {
                return rulesResult;
            }
